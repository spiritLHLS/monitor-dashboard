  ...
  <div>
      ...
      <el-form ref="elSearchFormRef" :inline="true" :model="searchInfo" class="demo-form-inline" :rules="searchRule"
        ...
        <template v-if="showAllQuery">
          ...
          <el-form-item label="短代码" prop="shortCode">
            <el-input v-model="searchInfo.shortCode" placeholder="搜索条件" />
          </el-form-item>
        </template>
        <el-form-item>
    ...
    <div class="gva-table-box">
      <div class="gva-btn-list">
        ...
        <el-button type="success" icon="link" style="margin-left: 10px;" :disabled="!multipleSelection.length"
          @click="generateEncryptedUrls">
          生成加密链接
        </el-button>
      </div>
      <el-table ref="multipleTable" style="width: 100%" tooltip-effect="dark" :data="tableData" row-key="ID"
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="330">
          <template #default="scope">
            ...
            <el-button type="primary" link class="table-button" @click="handleRedirectFunc(scope.row)">
              <el-icon style="margin-right: 5px">
                <Link />
              </el-icon>跳转
            </el-button>
            <el-button type="primary" link icon="edit" class="table-button"
              @click="updateEncryptedLinkFunc(scope.row)">变更</el-button>
...
import { QuestionFilled, InfoFilled, Link } from '@element-plus/icons-vue'
...
// 跳转
const handleRedirectFunc = async (row) => {
  try {
    const response = await handleRedirect({ shortCode: row.shortCode })
...
// 生成选中记录的加密链接
const generateEncryptedUrls = async () => {
  if (multipleSelection.value.length === 0) {
    ElMessage({
      type: 'warning',
      message: '请选择要生成加密链接的记录'
    })
    return
  }
  const ids = multipleSelection.value.map(item => item.ID)
  try {
    const res = await buildEncryptedUrl({ ids })
    if (res.code === 0) {
      ElMessage({
        type: 'success',
        message: '加密链接生成成功'
    ...
    console.error('生成加密链接时发生错误:', error)
    ElMessage({
      type: 'error',
      message: '生成加密链接时发生错误'
    })
